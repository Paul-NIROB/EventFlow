...
const Register = () => {
  useEffect(() => {
    document.title = "EventFlow — Register";
  }, []);
    ...
    <div className="max-w-md mx-auto py-20">
      <div className="glass p-8 md:p-10 rounded-3xl space-y-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-text/60">Join EventFlow and start exploring amazing events.</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            <label className="text-sm font-bold text-accent uppercase tracking-widest ml-1">Full Name</label>
            <input 
              type="text" 
              name="name"
              placeholder="John Doe" 
              className={`w-full bg-white/5 border ${errors.name ? 'border-red-500' : 'border-white/10'} rounded-2xl px-5 py-4 focus:outline-none focus:border-accent transition-colors`}
              value={formData.name}
        ...
        <p className="text-center text-text/60">
          ...
          <Link to="/login" className="text-accent font-bold hover:underline">Login</Link>
        </p>
      </div>
    </div>
  );
